...
public abstract class NodeFactory
{
    ...
    /// <summary>
    /// Remove all scene registrations whose target node type belongs to the given assembly.
    /// Called during hot reload cleanup so stale scene → type mappings don't persist.
    /// Also clears logged-conversion tracking for those paths so re-registered scenes
    /// get fresh log entries.
    /// </summary>
    internal static void RemoveByAssembly(System.Reflection.Assembly asm)
    {
        foreach (var (path, type) in _sceneTypes)
        {
            if (type.Assembly == asm)
            {
                _sceneTypes.TryRemove(path, out _);
                _loggedConversions.TryRemove(path, out _);
            }
        }
    }
