...
public class AsyncExample
{
    ...
    public async Task Foo(int i)
    {
        await Bar(i);
    }

    public async Task Bar(int i)
    {
        await Foo(i);
        await Foo(i);
    }
    
    public async Task What(int i)
    {
        what = await Woo(i);
    }
    
    public async Task<int> Whatever(int i)
    {
        what = await Woo(i);
        return await Woo(what);
    }

    public async Task<int> Wee(int i)
    {
        return await Woo(i);
    }

    public async Task<int> Woo(int i)
    {
        return await Wee(i);
    }

    public async Task<int> Waah(int i)
    {
        i = await Wee(i);
        i += 2;
        return await Woo(i);
...
public class BoxingTest()
{
    ...
    public void Add(ExampleStruct example)
    {
        var asObject = (object)example;
        dict[asObject] = example;

        ExampleStruct result = (ExampleStruct)dict[asObject];
        int i = result.I;
        i.ToString();
    }
    
    public struct ExampleStruct(int i)
    {
        public int I { get; } = i;
    }
}